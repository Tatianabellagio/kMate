...
**`kMate`**: per-sample k-mer Poisson EM on the 231-founder simplex, projected through a founder × variant matrix (`var_pa_231_arch3`):

```
c_k ~ Poisson( λ · h^T · kmer_pa[:, k] )   for each panel k-mer k
...
1. **Recombination violates the "every haplotype = a clean founder" assumption.** Evolved GrENE-Net samples have 1–3 generations of recombination → per-block ancestry. Production for the heavily-selfing GrENE-Net cohort is `--unit chrom` (one mixture per chromosome). For genuinely recombinant pools, a finer unit (`--unit bp`) can help; the anchored+smoothed 'star2' window recipe (global-anchor + HMM-smooth) is now an **opt-in** behind `--no-local-only`, not the default (windows are fit local-only). In the recombinant benchmarks the star2 window_200kb recipe won on the hardest n50_g3 regime by a narrow margin.
2. **Cactus-vs-PG k-mer asymmetry (+41% cactus h-bias).** Cactus founders have richer k-mer fingerprints (CV 2.6% across founders) than PanGenie-genotyped founders. The EM weighs k-mer evidence and over-credits cactus founders. Hurts the ~0.58% of records where carrier rates differ extremely (PG-specific variants, mostly Chr1q knob Mb 21-23). Mitigation strategies explored in `old_docs/BALANCING_KMERS.md`; the k-mer filter `filt2inv` is **decided** (chosen 2026-05-27, invariant-drop added 2026-05-29). The EM weight $\omega_k=1/m_b$ was the pre-fix mitigation; it remains the **window-mode** weight, but for **global mode** (the production estimator) it was **superseded 2026-07-06** by the per-founder M-step normalization + `--kmer-weight uniform` (which removes the imbalance at its source). See `docs/FOUNDER_NORMALIZATION_FIX.md` and `docs/PIPELINE_STATE.md` §0.
...
6. **Centromere alignment dead zone (Chr1 Mb 14–17 and pericentromeric regions on all chroms).** Structural; not fixable at EM or `var_pa` level. ~30% of per-SNP outliers live there.
...
- **Production kmer_pa matrices**: var_pa — `panel/arch3/chr{N}/var_pa_231_arch3_chr{N}.{var_pa,var_called,meta}.npz` (Arch 3); kmer_pa — `data/kmer_pa_231_arch3_filt2inv/` (in-house index + filt2inv). The k-mer-filter decision is **closed** (filt2inv); global-mode production weighting is now per_founder + `--kmer-weight uniform` (window keeps $\omega_k=1/m_b$; superseded 2026-07-06 for global mode — see `docs/FOUNDER_NORMALIZATION_FIX.md`). See `docs/PIPELINE_STATE.md` §0. The earlier candidate builds under `data/kmer_pa_231_v3qc_v3*` (raw, `_filt2`, `_mixedloose`, `_subsamp*`, …) and `kmer_pa_231_v3`/`var_pa_231_v3` are all *(archived)*.
