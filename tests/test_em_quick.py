...
def gen_synth(F, K, h_true, coverage, seed=0):
    ...
    mu = coverage * (h_true @ kmer_pa)
    counts = rng.poisson(np.maximum(mu, 1e-6))
    return kmer_pa, counts
...
def main():
    ...
    kmer_pa, counts = gen_synth(F, K, h_true, cov, seed=1)
    t = time.time()
    h_em, info = solve_em(counts, kmer_pa, cov, max_iter=200)
    ...
    kmer_pa, counts = gen_synth(F, 18523, h_true, 18.6, seed=2)
    t = time.time()
    h_em, info = solve_em(counts, kmer_pa, 18.6, max_iter=200)
    r2, rmse = metrics(h_em, h_true)
    print(f"  EM:  r²={r2:.4f}  rmse={rmse:.4f}  iters={info['iterations']}  [{time.time()-t:.2f}s]")
    h_wls, _ = solve_block_wls(counts, kmer_pa, 18.6)
    r2, rmse = metrics(h_wls, h_true)
    print(f"  WLS: r²={r2:.4f}  rmse={rmse:.4f}")

    print("\n--- T3: F=82, K=18523, cov=10, UNIFORM truth (LOW COV) ---")
    kmer_pa, counts = gen_synth(F, 18523, h_true, 10, seed=3)
    t = time.time()
    h_em, info = solve_em(counts, kmer_pa, 10, max_iter=200)
    r2, rmse = metrics(h_em, h_true)
    print(f"  EM:  r²={r2:.4f}  rmse={rmse:.4f}  iters={info['iterations']}  [{time.time()-t:.2f}s]")
    h_wls, _ = solve_block_wls(counts, kmer_pa, 10)
    r2, rmse = metrics(h_wls, h_true)
    print(f"  WLS: r²={r2:.4f}  rmse={rmse:.4f}")
    ...
    kmer_pa, counts = gen_synth(F, 18523, h_true, 10, seed=4)
    t = time.time()
    h_em, info = solve_em(counts, kmer_pa, 10, max_iter=200)
    r2, rmse = metrics(h_em, h_true)
    print(f"  EM:  r²={r2:.4f}  rmse={rmse:.4f}  iters={info['iterations']}  [{time.time()-t:.2f}s]")
    ...
    kmer_pa, counts = gen_synth(F, 50000, h_true, 10, seed=5)
    t = time.time()
    h_em, info = solve_em(counts, kmer_pa, 10, max_iter=200)
    r2, rmse = metrics(h_em, h_true)
    print(f"  EM:  r²={r2:.4f}  rmse={rmse:.4f}  iters={info['iterations']}  [{time.time()-t:.2f}s]")
