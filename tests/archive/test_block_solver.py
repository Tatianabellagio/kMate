...
def gen_synth(F=20, K=200, n_samples=1, coverage=30, seed=0,
    ...
    Returns:
        ...
        kmer_pa:       F × K (random binary presence/absence, ~50% allele freq)
        omega:    K-vector of true contamination rates
    """
    rng = np.random.default_rng(seed)
    # Random binary kmer_pa matrix; mean carrier freq ~0.3 to 0.5
    af = rng.uniform(0.1, 0.5, K)
    kmer_pa = (rng.random((F, K)) < af[None, :]).astype(np.int8)
    ...
    # Expected counts
    mu = coverage * (h_true @ kmer_pa) + coverage * omega[None, :]
    mu = np.maximum(mu, 1e-6)
    counts = rng.poisson(mu)
    return h_true, counts, kmer_pa, omega


def test_clean_overdetermined():
    """T1+T2: clean k-mers, K >> F. Should recover h tightly."""
    h_true, counts, kmer_pa, omega = gen_synth(F=20, K=500, coverage=50, seed=1)
    h_hat, _ = solve_block_wls(counts[0], kmer_pa, coverage=50.0)
...
def test_under_determined():
    ...
    h_true, counts, kmer_pa, _ = gen_synth(F=80, K=40, coverage=30, seed=2)
    h_hat, _ = solve_block_wls(counts[0], kmer_pa, coverage=30.0)
...
def test_known_omega():
    ...
    h_true, counts, kmer_pa, omega = gen_synth(F=20, K=500, coverage=50,
                                           contamination_frac=0.3, contamination_rate=5.0, seed=3)
    h_hat, _ = solve_block_wls(counts[0], kmer_pa, coverage=50.0, omega=omega)
    err = np.linalg.norm(h_hat - h_true[0])
...
def test_alternate_omega():
    ...
    h_true, counts, kmer_pa, omega_true = gen_synth(F=F, K=K, n_samples=N, coverage=50,
    ...
    for outer in range(3):
        for i in range(N):
            h_hat[i], _ = solve_block_wls(counts[i], kmer_pa, coverage=coverage[i], omega=omega_hat)
        omega_hat = update_omega(counts, kmer_pa, h_hat, coverage)
...
def test_coverage_invariance():
    ...
    h_true, counts1, kmer_pa, _ = gen_synth(F=20, K=500, coverage=20, seed=5)
    h_true2, counts2, _, _ = gen_synth(F=20, K=500, coverage=80, seed=5)  # same seed → same h_true
    assert np.allclose(h_true, h_true2)
    h1, _ = solve_block_wls(counts1[0], kmer_pa, coverage=20.0)
    h2, _ = solve_block_wls(counts2[0], kmer_pa, coverage=80.0)
...
def test_irls_vs_anscombe():
    ...
    h_true, counts, kmer_pa, _ = gen_synth(F=20, K=500, coverage=50, seed=7)
    h_a, _ = solve_block_wls(counts[0], kmer_pa, coverage=50.0)
    h_i, _ = solve_block_irls(counts[0], kmer_pa, coverage=50.0)
