"""
End-to-end test: real cn matrix from build_kmer_cn + synthetic Poisson counts
...
def main():
    ...
    F, K = cn_sparse.shape
    print(f"Loaded cn: {F} founders × {K:,} k-mers")
    print(f"  cn density: {cn_sparse.nnz / (F*K):.2%}")
    ...
    # Solve via WLS
    print("\n--- WLS solve ---")
    h_wls, obj_wls = solve_block_wls(counts, cn, coverage=lam)
    ...
    # Solve via IRLS
    print("\n--- IRLS solve ---")
    h_irls, obj_irls = solve_block_irls(counts, cn, coverage=lam, max_iter=8, verbose=False)
    ...
    print(f"\nResidual RMSE (counts - λ·cn.T·h_wls): {rmse:.2f}  (vs sqrt(λ)={np.sqrt(lam):.2f} expected from Poisson)")
