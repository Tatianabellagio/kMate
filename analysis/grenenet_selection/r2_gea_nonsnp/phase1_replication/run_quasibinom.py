...
PRODUCTION binomial model for the consolidated clq0.9 phase-1 pipeline (the
`pval_quasi` route is the settled decision, see STATUS_clq90.md §0B). Ported
verbatim from the now-archived gea_newpanel/run_quasibinom_latent.py; only the
default --out dir was repointed into phase1_replication/results/clq90/quasibinom.
...
def main():
    ...
    ap.add_argument("--out", default=f"{lib.GEA}/r2_gea_nonsnp/phase1_replication/results/clq90/quasibinom")
