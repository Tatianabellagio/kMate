#!/usr/bin/env python
"""GIF-corrected twin of the SV-peak x climate-axis dot grid (sv_peak_dotgrid_lfmm.ipynb).
...
So an SV "peak" here = lead SV record Bonferroni-sig AFTER this correction. Colour still =
max LD r2 of the SV to a SNP within 50 kb (masked, no-missingness `sv_snp_ld_v2_maconly`),
ring still = SV peak not shared by a SNP peak. Two figures: all peaks (dense) + top-45.
"""
...
md("""# SV peaks x climate axes — GIF-corrected dot grid (SNP-tagging view)

GIF-corrected twin of `sv_peak_dotgrid_lfmm.ipynb`. Same raw LFMM K=16 p-values, but
...
**Rows** = SV peak blocks (lead SV record Bonferroni-sig *after correction* on >=1 axis).
**Columns** = 20 climate axes. **Dot size** = corrected SV -log10p; **colour = max LD r2 of
the lead SV to a SNP within +/-50 kb** (masked `sv_snp_ld_v2_maconly`: dark = tagged by a
nearby SNP; pale = not); **black ring** = SV peak not shared by a SNP peak (SV Bonf-sig,
...
print(f"{len(peaks)} SV peak blocks (lead SV Bonferroni-sig AFTER GIF on >=1 of 20 axes)")''')
...
def draw(peaks_sub, pitch, dot_base, dot_scale, yfont, ringlw):
    ...
    ax.annotate(f"GIF-corrected LFMM \\u00b7 SV \\u2014 size \\u221d SV \\u2212log10p    \\u25cb = SV peak not shared by a SNP peak "
...
md("""## All SV peaks surviving GIF correction (dense)""")
...
tab.to_csv(f"{OUTDIR}/sv_peak_dotgrid_lfmm_gifcorr.csv", index=False)
print("wrote", f"{OUTDIR}/sv_peak_dotgrid_lfmm_gifcorr.csv", "|", len(peaks), "peaks x", len(AXES), "axes")''')

nb = new_notebook(); nb["cells"] = C
out = os.path.join(HERE, "sv_peak_dotgrid_lfmm_gifcorr.ipynb")
