#!/usr/bin/env python
"""BH-significant LD blocks -> gene table, across the completed last-gen GEA slice.

For every WZA output in {kendall,lfmm,binomial} x {snp,smallindel,sv} at gen9 / bio1 /
deg7-cap2000, compute BH-FDR on the block p (Z_pVal) and take the blocks with q < --fdr.
For each significant block:
  - reconstruct its genomic SPAN from the hapFIRE LD-block SNP map (lib.LD_BLOCKS),
  - list every TAIR10 gene overlapping that span (local GFF, lib.load_genes),
  - enrich each gene AT-id with its SYMBOL + DESCRIPTION via the Ensembl Plants REST API
...
Output (analysis/grenenet_selection/r2_gea_nonsnp/phase1_replication/results/):
  significant_genes_gen9_bio1_deg7cap2000.csv  one row per (block, gene):
    ...
    + 9 indicator cols  {model}_{class}  = BH q-value where that combo is significant.

  PY=/global/home/users/tbellg/miniforge3/envs/kmate/bin/python   # needs outbound HTTPS
  $PY analysis/grenenet_selection/r2_gea_nonsnp/phase1_replication/build_significant_genes.py --fdr 0.05
"""
...
def block_spans():
    """chrom('Chr#'), start, end, n_snp for every hapFIRE LD block."""
    b = pd.read_csv(lib.LD_BLOCKS, usecols=["pos", "chrom", "block"])
    b["block"] = b["block"].astype(str)
    g = b.groupby("block").agg(chrom=("chrom", "first"), start=("pos", "min"),
                               end=("pos", "max"), n_snp=("pos", "size"))
    g["chrom"] = "Chr" + g["chrom"].astype(str)
    return g
...
def main():
    ...
    for m in MODELS:
        for c in CLASSES:
            ...
            counts[(m, c)] = len(s)
            print(f"{m:9s} {c:11s}: {len(w):,} blocks | BH q<{args.fdr}: {len(s)}", flush=True)

    # 2) union of significant blocks, with which combos flagged each
    all_blocks = sorted({b for d in sig.values() for b in d})
    print(f"\nUnion of BH-significant blocks across all 9 combos: {len(all_blocks)}", flush=True)
    if not all_blocks:
    ...
    for blk in all_blocks:
        ...
        block_meta[blk] = (sp.chrom, int(sp.start), int(sp.end), int(sp.n_snp))
    ...
    for blk in all_blocks:
        ...
        for gid in gids:
            ...
            row = dict(gene=gid, symbol=symbol, description=desc, block=blk,
                       ...
                       n_genes_in_block=len(block_genes[blk]),
