#!/usr/bin/env python
"""Build raw-LFMM non-SNP-peak x climate-axis dot grid (companion to raw_manhattan_snp_vs_nonsnp_lfmm).

Rows = non-SNP peak blocks (a clq0.9 block whose lead non-SNP record clears per-record Bonferroni
...
(block, axis):
  ...
  - dot colour = SNP -log10p at the same block (pale = no SNP signal there = genuinely
    non-SNP-only; dark = a SNP tags the same block)
...
md("""# non-SNP peaks x climate axes — raw-LFMM dot grid (SNP-tagging view)

Companion to `raw_manhattan_snp_vs_nonsnp_lfmm.ipynb`, same raw (before-WZA) per-record
LFMM K=16 p-values. **Rows** = non-SNP peak blocks (lead non-SNP record clears per-record
...
dark = the variant is in tight LD with a nearby SNP -> taggable; pale = no nearby SNP tracks
...
genuinely-novel-nonSNP candidate: SV-significant, no co-significant SNP, **and** not even
...
# grid[(block, axis)] = (sv_nlp, sv_sig, snp_sig, r2)  ; r2 = lead non-SNP record's LD tag to a nearby SNP
grid = {}; recur = {}
for axis in AXES:
...
print(f"{len(peaks)} non-SNP peak blocks (lead non-SNP Bonferroni-sig on >=1 of 20 axes)")

CAP = None            # show ALL non-SNP peak blocks (no cap), packed dense
...
def draw(peaks_sub, pitch, dot_base, dot_scale, yfont, ringlw):
    ...
    ax.annotate(f"raw LFMM \\u00b7 non-SNP \\u2014 size \\u221d non-SNP \\u2212log10p    \\u25cb = non-SNP peak not shared by a SNP peak "
...
# ---- Section: top-150 most recurrent (dense) ----
md("""## Non-SNP peaks — top 150 most-recurrent (dense)

Non-SNP raw has **~671** peak blocks (far more than SV's 144) — too many to render
legibly, so this dense view shows the **150 most-recurrent** (most-recurrent at the
bottom); the full set is in `nonsnp_peak_dotgrid_lfmm.csv`.""")
co("""DENSE_N = 150
print(f'dense view: showing {min(DENSE_N,len(peaks))} of {len(peaks)} non-SNP peaks (full set in CSV)')
...
Same grid restricted to the 45 non-SNP peaks significant on the most axes — the readable
summary view.""")
...
tab.to_csv(f"{OUTDIR}/nonsnp_peak_dotgrid_lfmm.csv", index=False)
print("wrote", f"{OUTDIR}/nonsnp_peak_dotgrid_lfmm.csv", "|", len(peaks), "peaks x", len(AXES), "axes")''')

nb = new_notebook(); nb["cells"] = C
out = os.path.join(HERE, "nonsnp_peak_dotgrid_lfmm.ipynb")
