...
1. **Per-variant temporal selection — a real, small, SV-specific signal.**
   SV **insertions** show a small excess of purifying / parallel selection that is
   **climate-graded** (aridity-dominant, bio18). Three independent methods agree: climate-slope
   β (KS p≈10⁻¹⁶), ρ-parallelism across replicate plots (1.7–3.6× enrichment among parallel
   responders), and PicMin (1.2–1.3× among repeated-adaptation loci). Insertion-driven,
   robust to MAF, SNP-baseline-flat. **Two live caveats:** (i) global-mode AF can't separate
   SV-specific selection from **hitchhiking** on climate-purged haplotypes; (ii) the
   insertion polarity could be a systematic reference/mapping bias — *not* a within-panel
   calling-confidence artifact (that was tested and ruled out), but a true outgroup is still
   needed to fully close it. → **`SV_TEMPORAL_PURGING_SUMMARY.md`**
...
**Net:** there is no SV-specific *climate-GEA* or *block-enrichment* signal, but there **is** a
real per-variant *temporal* SV-insertion selection signal (climate-graded, 3 methods) — with
hitchhiking and reference-polarity caveats still open.
