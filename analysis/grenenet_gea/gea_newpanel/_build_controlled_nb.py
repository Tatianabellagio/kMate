#!/usr/bin/env python
"""Build the 'controlling the inflation' notebook for binomial + Kendall (SNP vs non-SNP):
two ways to tame the plot-level over-precision, both mirroring what LFMM does.

  (A) Honest 31-SITE unit   -- one climate value = one observation (fixes pseudoreplication
                               at the source); the same unit the LFMM site-PC1 panel uses.
  (B) Latent-factor binomial -- K=16 pool structure factors as GLM covariates (LFMM's U*V,
                               inside the binomial). Kendall has no covariate slot, so (B)
                               is binomial-only; a linear+latent model would just be LFMM.
...
C.append(new_markdown_cell(
    ...
    "The plot-level raw binomial/Kendall (bio1, gen9) are wildly inflated (binomial GIF≈105, "
    "Kendall GIF≈9) from two sources: **pseudoreplication** (355 pools carry only 31 independent "
    "climate values) and, for the binomial, **count over-precision** (AF×flowers×2 treated as "
    "independent draws). Genomic control alone flattened everything to 0 hits. Here are the two "
    "principled fixes, each the analog of an LFMM ingredient:\n\n"
    "- **(A) Honest 31-site unit** — flower-weighted site-mean AF; one obs per climate value. "
    "Removes pseudoreplication at the source (Kendall becomes genuinely calibrated; binomial "
    "loses the 355→31 inflation but keeps some count over-precision → still GIF-check it).\n"
    "- **(B) Latent-factor binomial (K=16)** — top-16 pool structure factors (PCs of the class "
    "AF matrix = LFMM's `U`) added as GLM covariates, so the climate slope is tested net of "
    "structure. This is LFMM's engine dropped into the binomial. **Kendall takes no covariates** "
    "(rank correlation, no design matrix), so (B) is binomial-only."))
...
# ---------- genome coords helper ----------
C.append(new_markdown_cell("Genome-coordinate + loader helpers (shared by both sections)."))
C.append(new_code_cell(
    "def add_gx(D, off):\n"
...
# ---------- Section A: site-level ----------
C.append(new_markdown_cell(
    "## (A) Honest 31-site unit — binomial + Kendall\n"
    "Flower-weighted site-mean AF, one observation per climate value (31 sites). "
    "`site_maf≥0.05`. This removes pseudoreplication; compare GIF here to the plot-level ≈105/9."))
C.append(new_code_cell(
    "site={}\n"
...
C.append(new_markdown_cell("### Site-level Manhattans (binomial, then Kendall)"))
C.append(new_code_cell(
...
C.append(new_markdown_cell("### Site-level top hits (annotated) — Kendall (the calibrated test)"))
C.append(new_code_cell(
...
C.append(new_markdown_cell("Kendall site-level — top SNP records"))
C.append(new_code_cell("site_top('kendall','snp')"))

# ---------- Section B: latent-factor binomial ----------
C.append(new_markdown_cell(
    "## (B) Latent-factor binomial (K=16) — LFMM's structure covariate inside the binomial\n"
    "Plot-level binomial GLM with the top-16 pool structure factors (PCs of the class AF matrix, "
    "= LFMM's `U`) added as covariates: `[alt,ref] ~ const + z(bio1) + LF1..LF16`. Compare GIF and "
    "hits to the **raw** plot-level binomial (no factors). If latent factors are doing LFMM's job, "
    "the GIF should fall sharply toward 1."))
C.append(new_code_cell(
    "lf={}; raw={}\n"
    "for cls in ['snp','nonsnp']:\n"
...
C.append(new_markdown_cell("### Latent-factor binomial Manhattan + top hits"))
C.append(new_code_cell(
...
# ---------- Section C: cross-reference the LFMM hit ----------
C.append(new_markdown_cell(
    "## (C) Do the controls recover the LFMM non-SNP hit?\n"
    "The LFMM site-PC1 panel's real non-SNP Bonferroni hit was **`Chr1_9323` @ Chr1:13,979,249** "
    "(2-bp indel). Check its p under each controlled binomial/Kendall scan — a genuine signal "
    "should survive the honest unit and stay non-significant in SNPs."))
C.append(new_code_cell(
...
C.append(new_markdown_cell(
    "## Read-out\n"
    "- **(A) Site unit** is the real pseudoreplication fix: Kendall on 31 sites is nonparametric "
    "and genuinely calibrated (GIF near 1); binomial on 31 sites drops the 355→31 inflation but "
    "keeps residual count over-precision (its site-level GIF, printed above, says how much). The "
    "site-level tables are the honest SNP-vs-non-SNP comparison, directly parallel to LFMM site-PC1.\n"
    "- **(B) Latent factors** are LFMM's actual engine, and they slot into the binomial: the GIF "
    "before/after quantifies how much of the inflation was structure the K=16 factors absorb. "
    "**Kendall cannot take covariates** — for a structure-corrected rank test you'd move to the "
    "site unit or partial correlation; a linear model of Δp with latent factors *is* LFMM.\n"
    "- **(C)** shows whether the controls preserve the LFMM non-SNP candidate. Convergence of "
    "site-level + latent-factor + LFMM on the same non-SNP loci (and their absence in SNPs) is "
    "the bar for 'signal SNPs miss' before any selection claim."))

nb = new_notebook(cells=C)
nb.metadata['kernelspec'] = {'name':'python3','display_name':'Python 3','language':'python'}
outp = '/global/scratch/users/tbellg/kmate/analysis/grenenet_gea/gea_newpanel/notebooks/binom_kendall_controlled.ipynb'
