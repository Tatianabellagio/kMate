...
OUTD = os.environ.get("PERMNULL_OUTD", f"{ROOT}/gea_newpanel/permnull_precip")
os.makedirs(OUTD, exist_ok=True)

# p0 store to use per class. There is no p0_sv.npy: sv records.col indexes p0_nonsnp.
P0_SRC = {"snp": "snp", "nonsnp": "nonsnp", "sv": "nonsnp"}

NSITES = 31
NPERM = int(os.environ.get("PERMNULL_NPERM", "2000"))
KS = [int(k) for k in os.environ.get("PERMNULL_KS", "1,3").split(",")]
AXES = os.environ.get("PERMNULL_AXES", "bio12,bio13,bio16,bio19").split(",")
CLASSES = os.environ.get("PERMNULL_CLASSES", "snp,nonsnp,sv").split(",")
MAF_MIN = 0.05
SEED = 12345
...
def load_class(cls):
    ...
    p0 = np.load(f"{ROOT}/af_store/p0_{P0_SRC[cls]}.npy").astype(np.float64)
...
def main():
    ...
    for cls in CLASSES:
        ...
        for K in KS:
            ...
            for axis in AXES:
                axis_h = int.from_bytes(hashlib.md5(axis.encode()).digest()[:4], "little")
                cls_h = int.from_bytes(hashlib.md5(cls.encode()).digest()[:4], "little")
                rng = np.random.default_rng(SEED + K * 100 + axis_h % 97 + cls_h % 13)
                x = std_env(axis)
