#!/usr/bin/env python
"""Precompute the gene-annotation table for the FDR-significant (q<0.05) SAMPLING-VARIANCE-
weighted LD-LMM blocks (site{SITE}_block_ld_lmm_sampvar.csv), so the notebook stays load-only.

Identical to _precompute_ldlmm_genes.py but points at the _sampvar model output and writes
site{SITE}_ldlmm_sampvar_genes.csv. Run in the `basic` env on a compute node (outbound HTTPS).
"""
import os
import sys
import pandas as pd
...
OUT = f"{H}/site{SITE}_ldlmm_sampvar_genes.csv"

d = pd.read_csv(f"{H}/site{SITE}_block_ld_lmm_sampvar.csv")
bonf = 0.05 / len(d)
