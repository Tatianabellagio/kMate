...
[ -s "$CN_VAR" ] || { echo "ERROR: missing cn_var (A5 not done)"; exit 1; }
...
print('[load] cn_var + cn_var_called')
t0 = time.time()
...
print(f'  cn_var founders (first 3): {cn_founders[:3]}')

# CRITICAL: align h to cn_var founder ordering, since h was built against a different
# panel build (v3qc_v3 cn_var) whose sample ordering may differ.
print('[align] verify h founders == cn_var founders, reindex if needed')
...
if h_founders_s == cn_founders_s:
    ...
    print('  ordering DIFFERS — reindexing h to cn_var founder order')
    ...
    if missing:
        raise SystemExit(f'ERROR: {len(missing)} cn_var founders missing from h: {missing[:5]}...')
    if extra:
        print(f'  WARN: {len(extra)} h founders not in cn_var (dropped): {extra[:5]}')
...
F = h.shape[0]
assert F == cn_var.shape[0], f'h has {F} entries but cn_var has {cn_var.shape[0]} founder rows'

print('[project] AF = h @ cn_var / h @ cn_var_called')
t0 = time.time()
numer = h @ cn_var          # shape (n_records,)
...
for tp, tr, ta in [(5870018,'T','A'), (10421645,'T','C'),
    ...
    if len(matches) == 0:
        print(f'  Chr1:{tp} {tr}>{ta}: NOT IN cn_var meta')
        continue
    for idx in matches:
