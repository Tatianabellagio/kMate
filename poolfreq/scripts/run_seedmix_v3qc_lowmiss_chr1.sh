...
#SBATCH --mem=80G
#SBATCH --time=2:00:00
#SBATCH --output=logs/sm_lowmiss_test_%j.out
#SBATCH --error=logs/sm_lowmiss_test_%j.err

mkdir -p logs
set -uo pipefail
cd /global/scratch/users/tbellg/hapfire_sv/poolfreq

OUT_DIR=/global/scratch/users/tbellg/hapfire_sv/scratch/seedmix_v3qc_lowmiss_test
mkdir -p $OUT_DIR
