#!/bin/bash
#SBATCH --job-name=mixstrict_v3
#SBATCH --partition=bse
#SBATCH --cpus-per-task=2
#SBATCH --mem=48G
#SBATCH --time=1:00:00
#SBATCH --output=/carnegie/nobackup/scratch/tbellagio/hapfire_sv/poolfreq/logs/mixstrict_v3_%j.out
#SBATCH --error=/carnegie/nobackup/scratch/tbellagio/hapfire_sv/poolfreq/logs/mixstrict_v3_%j.err
# Subset cn_full_v3qc_v3 to k-mers carried by ≥5 cactus + ≥5 PG founder.
set -euo pipefail
PY=/home/tbellagio/miniforge3/envs/hapfm/bin/python
OUT_DIR=/carnegie/nobackup/scratch/tbellagio/hapfire_sv/poolfreq/data/cn_full_231_v3qc_v3_mixedstrict
mkdir -p $OUT_DIR
...
CHR = 'Chr1'
SRC = Path('/carnegie/nobackup/scratch/tbellagio/hapfire_sv/poolfreq/data/cn_full_231_v3qc_v3')
OUT = Path('${OUT_DIR}')
...
keep = (ac_c >= 5) & (ac_p >= 5)
print(f'mixed-strict keep: {keep.sum():,}/{K:,} ({keep.mean()*100:.2f}%)', flush=True)
