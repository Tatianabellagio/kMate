...
for ci,(rlab,sim) in enumerate(COLS):
    ...
    for ri,(mlab,tag) in enumerate(METH):
        a=ax[ri,ci]; he=np.asarray(np.load(f"{ROOT}/scratch/h_fixes/replicates/h/{tag}__{sim}.npz",allow_pickle=True)["h"]).astype(float)
        ...
        a.hexbin(x,y,gridsize=80,bins="log",cmap="viridis",mincnt=1); a.plot([0,1],[0,1],"r--",lw=1)
...
fig.tight_layout(); out=f"{ROOT}/notebook/plots/af_true_vs_est_filt50.png"
