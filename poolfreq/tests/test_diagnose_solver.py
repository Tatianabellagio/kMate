...
Diagnostics:
  D1  cn matrix sanity: per-founder total k-mer count (Σ_k cn[f,k])
...
def main():
    ...
    print(f"\n--- D1: per-founder k-mer totals (Σ_k cn[f,k]) ---")
    per_founder = cn.sum(axis=1)
    ...
    print(f"\n--- D2: expected vs observed under uniform h ---")
    expected_uniform = cov * (cn.T @ h_true)  # K-vector
    ...
    # D4 unweighted L2
    h4 = solve_custom(counts, cn, cov, weights=np.ones(K))
    ...
    for it in range(8):
        mu = cov * (cn.T @ h_curr) + 1e-3
        weights = 1.0 / np.sqrt(mu)
        h_new = solve_custom(counts, cn, cov, weights=weights)
    ...
    h6 = solve_custom(counts, cn, cov, weights=weights6)
    report("D6  weights = 1/sqrt(AC)", h6)

    # D7 weights = 1/(AC + 1)
    weights7 = 1.0 / (ac + 1.0)
    h7 = solve_custom(counts, cn, cov, weights=weights7)
    report("D7  weights = 1/(AC+1)", h7)

    # D8 weights = sqrt(AC)
    weights8 = np.sqrt(ac.astype(float))
    h8 = solve_custom(counts, cn, cov, weights=weights8)
    report("D8  weights = sqrt(AC)", h8)

    # ============ D9: relative-frequency formulation ============
    # Drop the ω term and the absolute coverage; fit p_k = c_k / sum(c_k) vs cn^T h / sum(cn^T h)
    # i.e., regress relative frequencies, not absolute counts
    print(f"\n--- D9: relative-frequency regression ---")
    p_obs = counts / counts.sum()
    pred_normalizer = cn.sum(axis=0).sum()  # for unit h, sum(cn^T h) = sum(cn[f,:]) = scalar
    # Actually for arbitrary h, sum(cn^T h) = Σ_k Σ_f cn[f,k] h[f] = Σ_f h[f] Σ_k cn[f,k] = h^T per_founder
    # Use a non-normalized formulation:
    # p_pred = (cn^T h) / (h^T per_founder)
    ...
    h9 = solve_custom(counts, cn, cov, weights=np.ones(K))
    ...
    for c_try in [cov, cov_alt, cov * 0.5, cov * 2.0, cov * 5.0]:
        hh = solve_custom(counts, cn, c_try, weights=np.ones(K))
