...
$PY -u $ROOT/poolfreq/src/build_subsampled_cn.py \
    ...
    --out-prefix $ROOT/poolfreq/data/cn_full_231_v3qc_v3_subsampMedian_refilt2/cn_Chr1 \
    --target median --seed 42 --refilt2

echo
echo "=== 2. Subsample filt2 v3qc_v3 → median-per-stratum + refilt2 ==="
$PY -u $ROOT/poolfreq/src/build_subsampled_cn.py \
    --in-cn $ROOT/poolfreq/data/cn_full_231_v3qc_v3_filt2/cn_Chr1.cn.npz \
    --in-meta $ROOT/poolfreq/data/cn_full_231_v3qc_v3_filt2/cn_Chr1.meta.npz \
    --out-prefix $ROOT/poolfreq/data/cn_full_231_v3qc_v3_filt2_subsampMedian/cn_Chr1 \
    --target median --seed 42 --refilt2
...
$PY -u $ROOT/poolfreq/src/sweep_shape_norm_h_only.py \
    --cn-prefix $ROOT/poolfreq/data/cn_full_231_v3qc_v3_filt2_subsampMedian/cn_Chr1 \
    --reads $READS1 $READS2 \
    --sample SEEDMIX_S1 \
