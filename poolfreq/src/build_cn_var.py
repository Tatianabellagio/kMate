"""
Build cn_var: founder × biallelic-VCF-record matrices for AF projection.

Two parallel sparse matrices per record set:

  cn_var[f, r]        = 1 if founder f confirmed-carries the ALT at record r (GT 1/1 or any >0)
                        0 if confirmed REF (0/0) OR missing (./.)

  cn_var_called[f, r] = 1 if founder f's GT is NOT ./. at record r (called genotype)
                        0 if GT is ./.

Projection convention (handles missing properly — see SESSION_2026-05-18.md cn_var bug):
    AF_est[r] = (h @ cn_var)[r] / (h @ cn_var_called)[r]

For uniform h=1/F this reduces to AC/AN — the standard "AF among called samples".
The pre-fix convention (h @ cn_var, divide by 1) treats ./. as REF and systematically
...
Output: cn_var.npz, cn_var_called.npz, meta.npz (with chrom/pos/ref/alt strings).
"""
...
def build_cn_var(vcf_path: str, out_prefix: str, chrom_filter: str | None = None,
    ...
    for r_idx, rec in enumerate(iterator):
        ...
        for f_idx, sname in enumerate(samples):
            ...
            if gt is None or len(gt) == 0:
                # treat as ./.
                continue
            # Panel must be pre-haploidized; see build_kmer_cn.py for rationale.
            # Crash early on diploid GTs so cn_var doesn't silently disagree
            # with cn_full's GT[0]-only reconstruction.
            if len(gt) != 1:
                raise ValueError(
                    f"Panel VCF must be haploid (got len(GT)={len(gt)} for "
                    f"{sname} at {rec.chrom}:{rec.pos}). Haploidize the VCF "
                    f"before building cn_var."
                )
            # any None allele in GT marks the genotype as missing
            is_missing = all(a is None for a in gt)
            if is_missing:
                continue
            # genotype is called — mark in cn_var_called
    ...
    print(f"  cn_var carrier nnz: {len(carrier_rows):,}, "
          f"density: {len(carrier_rows)/(F*n_kept):.4f}")
    print(f"  cn_var_called nnz: {len(called_rows):,}, "
    ...
    print(f"Wrote {out_prefix}.cn_var.npz, {out_prefix}.cn_var_called.npz, "
