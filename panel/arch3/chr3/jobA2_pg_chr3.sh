...
# Phase 2 Job A2: PG-side full pipeline on full Chr3 (depends on A1).
...
echo "[$(date)] === Step 1: subset PG (all 153 samples) to full Chr3 ==="
PG_CHR1=pg_153_chr3.vcf.gz
if [ ! -s $PG_CHR1 ]; then
  $BCF view -r Chr3 $PG_RAW -Oz -o $PG_CHR1
  $TABIX -p vcf $PG_CHR1
fi
echo "  PG Chr3 records: $($BCF view -H $PG_CHR1 | wc -l)"
echo "  samples: $($BCF query -l $PG_CHR1 | wc -l)"

echo
echo "[$(date)] === Step 2: transfer INFO/ID from chr3 catalog ==="
PG_ANNOT=pg_153_chr3_annotated.vcf
...
PG_BIAL=pg_153_chr3_biallelic.vcf
...
PG_FILLED=pg_153_chr3_biallelic_filled.vcf.gz
...
N_HET_FLIPS_FILE=pg_153_chr3_het_flips.tsv
