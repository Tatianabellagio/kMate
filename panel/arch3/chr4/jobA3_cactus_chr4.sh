...
# Phase 2 Job A3: cactus-side full pipeline on full Chr4 (depends on A1).
...
CHR1_ANNOT=chr4_135_annotated.sorted.vcf.gz
BIAL_CATALOG=chr4_135_annotated_biallelic.sorted.vcf.gz
...
echo "[$(date)] === Step 1: subset cactus_78 to full Chr4 ==="
CACTUS_CHR1=cactus_78_chr4.vcf.gz
if [ ! -s $CACTUS_CHR1 ]; then
  $BCF view -r Chr4 $CACTUS78_RAW -Oz -o $CACTUS_CHR1
  $TABIX -p vcf $CACTUS_CHR1
fi
echo "  cactus_78 Chr4 records: $($BCF view -H $CACTUS_CHR1 | wc -l)"
echo "  samples: $($BCF query -l $CACTUS_CHR1 | wc -l)"

echo
echo "[$(date)] === Step 2: transfer INFO/ID from chr4 catalog ==="
CACTUS_ANNOT=cactus_78_chr4_annotated.vcf
$PY $TRANSFER --cactus $CHR1_ANNOT --pg $CACTUS_CHR1 --out $CACTUS_ANNOT

echo
echo "[$(date)] === Step 3: convert-to-biallelic ==="
CACTUS_BIAL=cactus_78_chr4_biallelic.vcf
...
CACTUS_HAP=cactus_78_chr4_haploid.vcf.gz
awk '$1 ~ /^#/ {print $0; next} {print $0 | "sort -k1,1 -k2,2n"}' $CACTUS_BIAL > cactus_78_chr4_biallelic.sorted.vcf
$BGZIP -f -c cactus_78_chr4_biallelic.sorted.vcf > $CACTUS_HAP
$TABIX -p vcf $CACTUS_HAP
echo "  $CACTUS_HAP: $(ls -lh $CACTUS_HAP | awk '{print $5}')"

echo
echo "[$(date)] DONE A3 (cactus Chr4)"
