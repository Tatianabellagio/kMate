...
# Phase 2 Job A2: PG-side full pipeline on full Chr2 (depends on A1).
...
echo "[$(date)] === Step 1: subset PG (all 153 samples) to full Chr2 ==="
PG_CHR1=pg_153_chr2.vcf.gz
if [ ! -s $PG_CHR1 ]; then
  $BCF view -r Chr2 $PG_RAW -Oz -o $PG_CHR1
  $TABIX -p vcf $PG_CHR1
fi
echo "  PG Chr2 records: $($BCF view -H $PG_CHR1 | wc -l)"
echo "  samples: $($BCF query -l $PG_CHR1 | wc -l)"

echo
echo "[$(date)] === Step 2: transfer INFO/ID from chr2 catalog ==="
PG_ANNOT=pg_153_chr2_annotated.vcf
...
PG_BIAL=pg_153_chr2_biallelic.vcf
...
PG_FILLED=pg_153_chr2_biallelic_filled.vcf.gz
...
N_HET_FLIPS_FILE=pg_153_chr2_het_flips.tsv
