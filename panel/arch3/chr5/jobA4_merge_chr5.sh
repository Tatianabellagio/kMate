...
# Phase 2 Job A4: merge cactus_78 + PG_153 (both Chr5 haploid biallelic) + post-merge monomorphic (AC=0|AC=AN) filter.
# Outputs the final 231-panel for Chr5.
...
CACTUS_HAP=cactus_78_chr5_haploid.vcf.gz
...
MERGED_FILLED=merged_231_chr5_filled.vcf.gz
...
MERGED_FINAL=merged_231_chr5_final.vcf.gz
...
total = 0
with gzip.open('merged_231_chr5_final.vcf.gz','rt') as f:
    for line in f:
        if line.startswith('##'):
            continue
...
print(f'\nSamples: {len(samples)}')
print(f'Total Chr5 records: {total:,}')
print()
...
for k in SPOT:
    if k not in results:
        print(f'  Chr5:{k[0]} {k[1]}>{k[2]}: not present')
        continue
    p = results[k]
    gts = p[9:]
    ac = sum(1 for g in gts if g == '1')
    an = sum(1 for g in gts if g in ('0','1'))
    print(f'  Chr5:{k[0]} {k[1]}>{k[2]}: AC={ac}/{an}  AF={ac/max(an,1):.4f}')
PYEOF

echo
echo "[$(date)] DONE A4 (merged Chr5 panel)"
ls -lh merged_231_chr5_final.vcf.gz
