...
# Phase 2 Job A3: cactus-side full pipeline on full Chr5 (depends on A1).
...
echo "[$(date)] === Step 1: subset cactus_78 to full Chr5 ==="
CACTUS_CHR1=cactus_78_chr5.vcf.gz
if [ ! -s $CACTUS_CHR1 ]; then
  $BCF view -r Chr5 $CACTUS78_RAW -Oz -o $CACTUS_CHR1
  $TABIX -p vcf $CACTUS_CHR1
fi
echo "  cactus_78 Chr5 records: $($BCF view -H $CACTUS_CHR1 | wc -l)"
echo "  samples: $($BCF query -l $CACTUS_CHR1 | wc -l)"

echo
echo "[$(date)] === Step 2: transfer INFO/ID from chr5 catalog ==="
CACTUS_ANNOT=cactus_78_chr5_annotated.vcf
$PY $TRANSFER --cactus $CHR1_ANNOT --pg $CACTUS_CHR1 --out $CACTUS_ANNOT

echo
echo "[$(date)] === Step 3: convert-to-biallelic ==="
CACTUS_BIAL=cactus_78_chr5_biallelic.vcf
...
awk '$1 ~ /^#/ {print $0; next} {print $0 | "sort -k1,1 -k2,2n"}' $CACTUS_BIAL > cactus_78_chr5_biallelic.sorted.vcf
$BGZIP -f -c cactus_78_chr5_biallelic.sorted.vcf > $CACTUS_HAP
$TABIX -p vcf $CACTUS_HAP
echo "  $CACTUS_HAP: $(ls -lh $CACTUS_HAP | awk '{print $5}')"

echo
echo "[$(date)] DONE A3 (cactus Chr5)"
