...
- **Modes**: `global` (one h per chrom) is **the GrENE-Net production estimator** (heavy selfing; emits both founder h and per-record AF). Window mode now defaults to **`--local-only`** (2026-07-07): global-free — no anchor prior, no global fallback (thin/empty windows → NaN AF), no cross-window smoothing; each window fit purely on its own k-mers over the haplotypes present. The legacy anchored+smoothed "star2" recipe (10kb + anchor 0.3 + HMM smooth 5α0.5) is now behind `--no-local-only`. Window mode is still **NOT in production use** for GrENE-Net — the existing `grenenet_kmate_window*` cohort outputs are **STALE** (old multinomial EM, not regenerated under the per_founder fix; decision 2026-07-07) and should not be used. The window-mode *code path* works if ever revived. See `docs/RERUN_AFTER_FIX.md` Group D. The raw stale outputs (~1.6 TB) were archived to `results/archive/` on 2026-07-07.
