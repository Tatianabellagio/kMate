...
A. thaliana is treated as effectively haploid because the GrENE-Net populations are >97% selfing (Exposito-Alonso et al. 2018); each individual carries one mosaic haploid genome. The two diploid copies in real individuals are near-identical, and `var_pa` is stored per founder, not per haplotype.
...
**Coordinate-drift caveat (corrected 2026-06-18).** Breakpoints are defined in TAIR10 coordinates but applied to *consensus* FASTAs, whose length drifts from TAIR10 by the cumulative indel offset. This drift is **not** "few bp": across the 231 founders the consensus Chr1 is shorter than TAIR10 by a median of ~204 kb and up to ~430 kb (81% of founders are shorter). Two consequences: (i) a fetch of the full TAIR10 span must be clamped to the founder's real sequence length — otherwise it overruns into the next chromosome's record (the pre-2026-06-18 builder spliced `>Chr2…` into the distal tail of every mosaic; `fa_fetch` now clamps and **N-pads** the deficit so the mosaic stays exactly the TAIR10 span and the tail yields no reads rather than wrong-chromosome reads); (ii) for recombinant individuals, TAIR10-coord breakpoints are mis-registered against the drifted consensus by the cumulative offset. For **pure-founder** individuals (all of g0, and the large majority under realistic selfing) the drift is immaterial because kMate is k-mer-based and position-independent; for recombinants in **global mode** the net effect on the chromosome-wide mixture is small and averages over the pool. An alignment-based fix (project each breakpoint through a consensus→TAIR10 map) is the same machinery deliberately scoped out for raw assemblies in §3.1 and is not applied here. See `sims/AUDIT_2026-06-18.md`.
...
**The risk (read-side closed loop).** Consensus reads contain only the variants the panel VCF encoded, and `kmer_pa` (the EM's k-mer dictionary) is built from that same VCF. So the simulated read substrate is a guaranteed subset of the estimator's k-mers: no read carries a k-mer `kmer_pa` hasn't seen. Real pool-seq reads do — real genomes carry variation and repeat content the pangenome graph never captured. If that off-panel k-mer mass degraded the EM, a consensus-only simulation would hide it.

**The validation (g0).** We tested this on the two non-recombinant regimes by re-simulating reads **straight from the raw assemblies** (`…/pang_1001gplus/…/chr_only/<asm_id>.chr.fa`, via `pywgsim`, bypassing the VCF→consensus→VISOR path entirely; `benchmarks/p80/scripts/sim_from_raw_assemblies.py`) and re-running `kMate global` against the *same* `kmer_pa`, `var_pa`, and truth. Only the read source differs. Both substrates used the same 10× read budget (verified: 9.95× consensus / 10.00× raw of TAIR10 Chr1). Raw reads carry extra off-panel k-mer mass relative to consensus — the EM's λ̂ coverage estimate read ~7.2× for raw vs ~7.6× for consensus from the *identical* budget (λ̂ counts only k-mers present in `kmer_pa`; the ~7.x value is itself a uniform-h / partly-represented-panel artifact seen for both substrates, not missing depth).
...
**Why we keep the VCF-consensus simulation (the recombination constraint).** For the recombinant regimes (g1, g3, dom500) the consensus path is not just convenient — it is required. A recombinant individual is a mosaic whose breakpoints are defined in **TAIR10 coordinates** (§2.3). Consensus FASTAs are approximately in TAIR10 coordinates (TAIR10 + that founder's variants), so a mosaic is stitched by slicing each consensus FASTA at the TAIR10 breakpoint. This is *approximate*, not exact: consensus length drifts from TAIR10 by the cumulative indel offset (median ~204 kb, up to ~430 kb on Chr1 — see §2.4 coordinate-drift caveat), so breakpoints are mis-registered by that offset and the fetch must be clamped + N-padded to avoid overrunning the chromosome. Raw assemblies are in their **own**, far more divergent coordinate frame (Chr1 ranges 29.5–34.6 Mb vs TAIR10's 30.4 Mb), so stitching a TAIR10-defined mosaic from raw sequence requires an explicit assembly→TAIR10 alignment (e.g. minimap2) to project every breakpoint. The consensus path keeps that drift to the few-hundred-kb scale instead of multi-Mb, but does not eliminate it. Since g0 establishes that the read substrate is **not** the accuracy bottleneck, that added machinery would buy nothing for the recombinant regimes. **We therefore use VCF-consensus reads throughout the regime sweep, validated against raw-assembly reads at g0.**
...
The truth file also reports `info[r] = Σ_i w_i · var_called[anc_i(r), r]` — the h-weighted fraction of pool mass observable at record `r`. At records where `info → 0` (no individual's ancestry-founder is called), `truth_af` is emitted as `NaN`.

Optionally (when `source_weights.tsv` is present), a second truth column `source_truth_af` reports the source-population AF (`p_f · var_pa` / `p_f · var_called`) — the population parameter the AF estimator is ultimately trying to recover. The gap `|truth_af − source_truth_af|` quantifies the Stage-1 sampling + drift noise floor — irreducible by any AF estimation method given the realised pool.
...
Regimes are ordered (in result tables) easiest → hardest by empirical realised-pool truth MAE. The same naming scheme generalises: any (`n<N>_g<G>` ± `_dom<F>`) tuple identifies a simulation. The 231-panel production benchmark uses the same regimes against the production `var_pa` and `kmer_pa` matrices.
...
For results, evaluation methodology, and the regime-by-method MAE tables, see `benchmarks/p80/results/FINAL_RESULTS_cov10_p80.ipynb` (notebook) and the `benchmarks/p80/results/plots/` figure set. The 231-panel regime sweep on the production var_pa/kmer_pa is the next planned step (see `PIPELINE_STATE.md` §3).
