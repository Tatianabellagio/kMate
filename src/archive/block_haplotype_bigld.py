"""ARCHIVED — experimental BigLD per-block-haplotype EM driver (bigld_haplotype mode).

NOT production. kMate production uses window (`star2`) + global modes
(see `src/per_sample_per_chrom.py`). This driver was a *tried* approach, retained
for provenance / the methods record (`docs/METHODS_TRIED_AND_RESULTS.md`).
It was previously the tail of `src/block_haplotype_em.py`; the still-live
`smooth_h_across_blocks` helper it calls remains there.
...
# Archived module: resolve sibling production modules in ../ for provenance only.
