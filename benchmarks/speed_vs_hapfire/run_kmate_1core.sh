...
echo "[$(date)] kMate benchmark start on $(hostname), ${SLURM_CPUS_PER_TASK:-?} cpus (threads pinned to 1)"

/usr/bin/time -v -o "$RES/kmate_time_1core.txt" $PY "$ROOT/src/per_sample_per_chrom.py" \
    ...
    --sample kmate_n231_chr1_cov50_f30_1core \
    --out kmate_n231_chr1_cov50_f30_1core.tsv \
    --threads 1 --chroms Chr1 --kmer-weight uniform --block-mode global
