...
#SBATCH --mem=64G
#SBATCH --time=8:00:00
#SBATCH --output=logs/03_kmer_pa_%j.out
#SBATCH --error=logs/03_kmer_pa_%j.err

# =============================================================================
# Phase A3 -- Build kmer_pa_p80/kmer_pa_Chr1.{kmer_pa,meta}.npz
# Path B (on-the-fly per-bubble reconstruction; matches v3qc kmer_pa build).
#
...
# index) would mismatch bubble topology and silently produce wrong kmer_pa
# reconstructions.
# =============================================================================
mkdir -p logs
set -euo pipefail
...
OUT_DIR=$CTRL/data/kmer_pa_p80
OUT_PREFIX=$OUT_DIR/kmer_pa_Chr1
mkdir -p $OUT_DIR
...
if [ -s "${OUT_PREFIX}.kmer_pa.npz" ] && [ -s "${OUT_PREFIX}.meta.npz" ]; then
    echo "[$(date)] kmer_pa_p80 already present -- skip"
    ls -lh ${OUT_PREFIX}.kmer_pa.npz ${OUT_PREFIX}.meta.npz
    exit 0
fi

echo "[$(date)] build kmer_pa_p80 (path B, on-the-fly reconstruction)"
echo "  kmers: $KMERS"
echo "  vcf:   $VCF"
echo "  ref:   $REF"
echo "  out:   ${OUT_PREFIX}.{kmer_pa,meta}.npz"
...
M = sp.load_npz("${OUT_PREFIX}.kmer_pa.npz")
