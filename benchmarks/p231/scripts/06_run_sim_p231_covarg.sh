...
#SBATCH --output=logs/06_simcv_%j.out
#SBATCH --error=logs/06_simcv_%j.err

# =============================================================================
# COV-parameterized sibling of 06_run_sim_p231.sh (mirrors p80's
# 06_run_sim_p80_covarg.sh): same everything, but COVERAGE reads from the COV
# env var (default 10) instead of being hardcoded, for the 1x/10x pool-size
# sweep (Fig S13-style benchmark). 06_run_sim_p231.sh is untouched/still
# cov10-only so existing invocations stay byte-identical.
#
# benchmarks/p231 Phase B -- recombinant sim on the 231 panel (Chr1).
# Mirrors benchmarks/p80/06_run_sim_p80.sh with TWO deliberate changes:
