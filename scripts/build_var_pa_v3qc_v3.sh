#!/bin/bash
#SBATCH --job-name=var_pa_v3qc_v3
#SBATCH --account=co_moilab
#SBATCH --partition=savio4_htc
#SBATCH --qos=moilab_htc4_normal
#SBATCH --cpus-per-task=2
#SBATCH --mem=128G
#SBATCH --time=2:00:00
#SBATCH --output=logs/var_pa_v3qc_v3_%j.out
#SBATCH --error=logs/var_pa_v3qc_v3_%j.err
mkdir -p logs
set -euo pipefail
BASE=/global/scratch/users/tbellg/kmate
PY=/global/home/users/tbellg/miniforge3/envs/hapfm/bin/python
VCF=$BASE/panel/pangenie_genotyping/data/v3qc_v3/founders_231_v3qc_v3.haploid.vcf.gz
OUT_PREFIX=$BASE/data/var_pa_231_v3qc_v3
[ -s "$VCF" ] || { echo "ERROR: missing $VCF"; exit 1; }
[ ! -s "${OUT_PREFIX}.var_pa.npz" ] || exit 0
$PY -u $BASE/src/build_var_pa.py --vcf "$VCF" --out "$OUT_PREFIX"
ls -lh ${OUT_PREFIX}.*
