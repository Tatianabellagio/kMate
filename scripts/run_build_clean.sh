...
#SBATCH --mem=96G
#SBATCH --time=4:00:00
#SBATCH --output=logs/build_clean_%j.out
#SBATCH --error=logs/build_clean_%j.err
...
if [[ "$MAX_BLOCKS" -gt 0 ]]; then
    ...
    OUT=data/block_haplotype_cn/chr1_first${MAX_BLOCKS}_clean.npz
else
    OUT=data/block_haplotype_cn/chr1_full_clean.npz
fi
