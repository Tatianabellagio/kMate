...
if [[ "$MAX_BLOCKS" -gt 0 ]]; then
    ...
    OUT=data/block_haplotype_cn/chr1_first${MAX_BLOCKS}_${SUFFIX}.npz
else
    OUT=data/block_haplotype_cn/chr1_full_${SUFFIX}.npz
fi

echo "[$(date)] starting build_block_haplotype_cn_cnvar (max_blocks=$MAX_BLOCKS)"
$PYTHON -u scripts/build_block_haplotype_cn_cnvar.py \
