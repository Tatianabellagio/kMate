...
Companion to `ALGORITHM.md` (prose). This file states the algorithm in formal math.
...
### PanGenie k-mer index caveats (inherited filters)

We build `cn` from PanGenie's `kmers.tsv.gz` directly, so PanGenie's index-time filtering is **inherited** by the cactus_em pipeline. Three filters apply per pangenome bubble:

1. **Internal var-dedup:** a k-mer must be unique within the bubble's alleles (`if (local_count > 1) continue`).
2. **Internal ref-dedup:** a k-mer must not appear elsewhere in the graph or reference (`if (genomic_count - local_count != 0) continue`).
3. **Hard cap:** **16 k-mers per allele** for biallelic bubbles, **32 per allele** for multi-allelic. Biallelic bubbles thus carry up to 32 k-mers (ref + alt); N-allelic bubbles up to $16N$.

Distribution observed in `cn_full_Chr1.meta.npz` (206,631 bubbles, 20.8M total k-mers, mean 100.6 / median 84 / max 301):
...
**Implication for coverage analysis:** with ≥4 k-mers/bubble (99.7% of bubbles) and ~80M unique k-mers genome-wide, the EM is massively over-determined for a 231-vector founder simplex even at 5× read coverage. This is consistent with the observed cov 5× ≡ cov 30× saturation. The cap removes redundancy, not signal — every bubble still has enough k-mers to discriminate its allele under the simplex prior.

**Implication for very low coverage (≪ 1×):** at ~0.5× whole-genome coverage, expected hits per k-mer ≈ 0.5; per allele with cap=16 → expected ≈8 hits per allele, which is the threshold where the cap can start to limit estimation. Realistic GrENE-Net evolved data is 5–15× so this is well above the cap regime.
