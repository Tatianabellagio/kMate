...
End-to-end description of how cactus_em estimates SNP and SV allele frequencies in pool-seq samples. Companion docs: `CACTUS_EM_MATH.md` (formal math), `BACKGROUND.md` (why we built it), `AF_TRUTH_VS_ESTIMATE_arch3_chr1.ipynb` (canonical Arch 3 validation), `FINAL_RESULTS_cov10_v3.ipynb` (simulation benchmark, archive).
...
For the 78 cactus founders (post v3qc 2-exclude), GTs come from the cactus pangenome VCF directly. For the 153 PG founders, GTs come from PanGenie against 1001G short reads. Beagle imputation was evaluated and hard-rejected for SVs (−25 to −30pp concordance loss); historical rationale in `old_docs/IMPUTATION_PLAN.md`.
...
└── data/
    ├── cn_full_231_v3qc_v3_mixedloose/cn_Chr1.cn.npz  # production k-mer matrix (Chr1)
    └── [archive: cn_full_231_v2/, cn_full_231_v3/, cn_full_231_v3qc/, cn_full_231_v3qc_v2/]

arch3/chr1/
    ├── cn_var_231_arch3_chr1.cn_var.npz           # production carrier matrix (231 × 2.6M)
    ├── cn_var_231_arch3_chr1.cn_var_called.npz    # called mask
    ├── cn_var_231_arch3_chr1.meta.npz
    ├── cn_var_231_arch3_chr1_atomized.*.npz       # per-base atomized (7.46M rows, SNP GEA)
    └── [archive: poolfreq/data/cn_var_231_v2, v3, v3qc, v3qc_v2, v3qc_v3]
```
